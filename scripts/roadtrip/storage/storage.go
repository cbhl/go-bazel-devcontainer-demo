...
// NewGCSClient creates a new GCS client (mock implementation for now)
func NewGCSClient(ctx context.Context, projectID, bucketName string) (*MockStorageClient, error) {
	return NewMockStorageClient(projectID, bucketName), nil
}

// NewMinIOClient creates a new MinIO client (mock implementation for now)
func NewMinIOClient(endpoint, accessKey, secretKey, bucketName string, useSSL bool) (*MockStorageClient, error) {
	return NewMockStorageClient("mock-project", bucketName), nil
}
